...
import activityHandler, { type ClickHouseHandlerCtx } from '~/aggregator/btc/handlers/activity-ch';
import { createSourceService } from '~/aggregator/btc/source-service-hex-addr';
import { type ResolvedBlock } from '~/aggregator/btc/types';
...
const PREPARATION_BUFFER_SIZE = Number(env.AGGREGATOR_PREPARATION_BUFFER_SIZE ?? 50);
const PREPARATION_CONCURRENCY = Number(env.AGGREGATOR_PREPARATION_CONCURRENCY ?? 1);
const HANDLER_RETRY_DELAY = 50;
const LOGGER_LEVEL = env.AGGREGATOR_LOG_LEVEL || 'info';
const ROLLBACK_WAIT_TIME = 30_000;

// initial dependencies
const blockService = createBlockService({ network: 'btc' });
...
const memoizedGetResolvedBlock = decorate(sourceService.getResolvedBlock.bind(sourceService), {
  maxCacheSize: PREPARATION_BUFFER_SIZE + PREPARATION_CONCURRENCY,
});
...
const storageScope = process.env.KV_SCOPE ?? 'btc-clickhouse';
    ...
    const batchHandler: BatchHandler<TxAssetDetailInsert[]> = {
      ...
      save: async (preparedDatum) => {
        ...
        preparedDatum
          ...
          .forEach((blockNumber) => memoizedGetResolvedBlockByNumber.delete(blockNumber));
      },
    };
